	...
	function read_comments ( $y, $m, $entry, $logged_in ) {
		...
		if ( $contents ) {	
			...
			for ( $i = 0; $i <= count( $contents ) - 1; $i++ ) {
				...
				$entry_array["subject"] = blog_to_html( $comment_subject, true, false );
				$entry_array["date"] = blog_to_html( format_date( $comment_entry_data["DATE"] ), true, false );
				$entry_array["entry"] = blog_to_html( $comment_entry_data["CONTENT"], true, false ) . "<br clear=\"all\">";
		...
		if ( $result ) {
			// Update Most Recent List
	 		...
	 		if ( $blog_config[ "blog_email_notification" ] ) {
				...
				$body="Posted by: " . $comment_name . "<br />\n";
				...
				$body=$body . '<a href="' . $base_url . 'comments.php?y=' . $y . '&m=' . $m . '&entry=' . $entry . '">' . $base_url . 'comments.php?y=' . $y . '&m=' . $m . '&entry=' . $entry . "</a><br /><br />\n\n";
				$body=$body . '<i>On ' . format_date( $comment_date ) . ', ' . $comment_name . " wrote:</i><br /><br />\n\n" . blog_to_html( $comment_text, false, false );
