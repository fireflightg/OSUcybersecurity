<?php 
			...
			for ( $i = 0; $i <= count( $contents ) - 1; $i++ ) {
				...
				if ( $blog_config[ 'blog_enable_voting' ] == true ) {
					...
					for ( $star_number = 1; $star_number <= 5; $star_number++ ) {
						...
						$str = $str . '<a href="javascript:self.location.href=\'rate_cgi.php?y=' . $y . '&m=' . $m . '&entry=' . $entry . '&rating=' . $star_number . '\';" title="' . $lang_string['sb_rate_entry_btn'] . '" alt="' . $lang_string['sb_rate_entry_btn'] . '"><img src="themes/' . $blog_theme . '/images/stars/' . $star_image . '" border="0"></a>';	
					}
				...
				if ( $blog_config[ 'blog_enable_comments' ] == true ) {
					...
					if ( $blog_config[ 'blog_comments_popup' ] == 1 ) {
						...
						$entry_array['comment']['url'] = 'javascript:self.location.href=\'comments.php?y='.$y.'&m='.$m.'&entry='.$entry.'\';';
					}
	...
	function get_latest_entry () {
							...
							while ( $subdirname != false ) {
								if ( $subdirname != '.' && $subdirname != '..' ) { 
									...
									if ( is_dir( $basedir.$dirname.'/'.$subdirname ) ) {
										...
										if ( intval( $dirname ) >= intval( $latest_y ) ) {
											$latest_y = $dirname;
											$latest_m = NULL;
										}
										if ( intval( $subdirname ) >= intval( $latest_m ) ) {
											$latest_y = $dirname;
											$latest_m = $subdirname;
		...
		if ( isset($latest_y) ) {
			$GLOBALS['year'] = $latest_y;
			$GLOBALS['month'] = $latest_m;
			$GLOBALS['day'] = $latest_d;
		}
	}
