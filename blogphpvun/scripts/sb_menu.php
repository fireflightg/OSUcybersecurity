<?php 
	...
	function read_menus_calendar ( $m, $y, $d ) {
		...
		if ( strpos( $lang_string['locale'], 'ES' ) !==false )
		{
		...
		for ( $i = 1; $i<=$total_days_in_month; $i++)
		{
			if ( $counts[$i-1] > 0 )
			{
				$str = $str . '<td align="center"><a href="index.php?d=' . sprintf( '%02d', $i) . '&m=' . sprintf( '%02d', $m ) . '&y=' . sprintf( '%02d', $y % 100 ) . '" title="' . $counts[$i-1] . '">' . $i . '</a></td>';
			}
			else
			{
		...
		$str = $str . '</tr></table>'; // Close the table
		return( $str );
	}
			...
			for ( $i = 0; $i < count( $array ); $i = $i + 4 ) {
				...
				if ( $comment_entry_data !== false) {
					...
					if ( $blog_config[ 'blog_comments_popup' ] == 1 ) {
						...
						$str_comments = $str_comments . '<a href="javascript:self.location.href=\'comments.php?y='.$y.'&m='.$m.'&entry='.$blog_entry_id.'\';">'.$comment_name.'</a><br />';
					}
